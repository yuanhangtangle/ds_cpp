...
class vector
{
    ...
    public:
        ...
        int _partition1(int lo, int high);
        int _partition2(int lo, int high);
        void quickSort1();
        void quickSort2();
        void _quickSort1(int lo, int high);
        void _quickSort2(int lo, int high);
        T quickSelect(Rank k);
};

template<typename T>
T vector<T>::quickSelect(Rank k){//the kth(k >= 1) largest element, 
    k = _size - k;
    int lo = 0, high = _size - 1, mid = -1, _lo, _high;
    T pivot;
    while(true){
        _lo = lo; _high = high;
        while(_lo < _high){
            pivot = _elem[lo];
            while(_lo < _high && _elem[_high] >= pivot) --_high;
            swap(_elem[_lo], _elem[_high]);
            while(_lo < _high && _elem[_lo] <= pivot) ++_lo;
            swap(_elem[_lo], _elem[_high]);
        }
        _elem[_lo] = pivot;

        if(k == _lo) return _elem[_lo];
        else if(k > _lo){
            lo = _lo + 1;
            k -= lo;
        }else{
...
template<typename T> 
int vector<T>::_partition1(int lo, int high){
    if(high <= lo) return lo;
    T pivot = _elem[lo];// random selection can be introduced
    while(lo < high){
        while(lo < high && _elem[high] >= pivot) --high;
        _elem[lo] = _elem[high];
        while(lo < high && _elem[lo] <= pivot) ++lo;
        _elem[high] = _elem[lo];
    }
    _elem[lo] = pivot;
    return lo;
...
template<typename T> 
int vector<T>::_partition2(int lo, int high){
    if(high <= lo) return lo;
    T pivot = _elem[lo];// random selection can be introduced
    int n = high; int mid;
    for(mid = high = lo+1; high <= n; high ++){
        if(_elem[high] < pivot) 
            swap(_elem[mid++], _elem[high]);
    }
    --mid; swap(_elem[lo], _elem[mid]); return mid;
}
template<typename T>
void vector<T>::_quickSort1(int lo, int high){
    if(high <= lo) return;
    int mid = _partition1(lo, high);
    _quickSort1(0, mid - 1); _quickSort1(mid + 1, high);
}
template<typename T>
void vector<T>::_quickSort2(int lo, int high){
    if(high <= lo) return;
    int mid = _partition2(lo, high);
    _quickSort2(0, mid - 1); _quickSort2(mid + 1, high);
}
template<typename T> 
void vector<T>::quickSort1(){
    this -> _quickSort1(0, _size - 1);
}

template<typename T> 
void vector<T>::quickSort2(){
    this -> _quickSort2(0, _size - 1);
}
...
vector<T>::vector(int c, int s, T v)
{
    ...
    for(_size = 0; _size < s; _elem[_size++] = v); //非常干净简练的书写风格 
}
...
//size, capacity, elem 全部搞定
{
    ...
    for(_size = 0; lo < hi; _elem[_size++] = v[lo++]);  //用for搞定 _size 处理以及复制
}
...
{
    int s;
    for(int i = 0; i < _size-1; i++) _elem[i] > _elem[i+1] ? s++:s;
    return s;
}
...
{
    int l = mi - lo; int r = hi - mi;
    for(int i = 0; i < l; i++) p[i] = _elem[i + lo];
    for(int i = 0; i < r; i++) q[i] = _elem[i + mi];
    int i, j, s;
    for(i = j = 0, s = lo; (i < l)||(j < r); s++)
    {
        if (i >= l) _elem[s] = q[j++];
        else if (j >= r) _elem[s] = p[i++];
        else if (p[i] <= q[j]) _elem[s] = p[i++];
        else _elem[s] = q[j++];
    }
}
...
void vector<T>:: _mergeSort2(Rank lo, Rank hi, T* p , T* q)
{
    if (hi - lo <= 1) return;
    Rank mi = (lo + hi) / 2;
    _mergeSort2(lo, mi, p, q); _mergeSort2(mi, hi, p ,q);
    merge2(lo, mi, hi, p, q);
}
