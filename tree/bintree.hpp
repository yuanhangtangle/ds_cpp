...
#define FromParentTo(x)\
    (IsRoot((x))? _root:(IsLChild((x))? (x).parent->lc: (x).parent->rc))//_root must be defined
...
class BinTree
{
    ...
    public:
        ...
        ~BinTree(){ if(_size > 0) remove(_root); } //``remove``?
        ...
        template<typename VST> void travPre_Recur(BinNodePosi(T) x, VST& visit);
        template<typename VST> void travIn_Recur(BinNodePosi(T) x, VST& visit);
        template<typename VST> void travPost_Recur(BinNodePosi(T) x, VST& visit);
        ...
        template<typename VST> void travPost_I1(BinNodePosi(T) x, VST& visit);
...
BinNodePosi(T) BinNode<T>::succ(){
    if(rc){
        BinNodePosi(T) bn = rc;
        while( bn -> lc ) bn = bn -> lc;
        return bn;
    }
    else{
        BinNodePosi(T) bn = this; //注意根节点的特殊性
        while( IsRChild( *bn ) ) bn = bn -> parent;
        return bn -> parent;
    }
}
...
BinNodePosi(T) BinNode<T>::pred(){
    ...
    if(lc){
        bn = lc;
        while(bn -> rc) bn = bn -> rc;
        return bn;
    }
    else{
        bn = this;
        while(IsLChild(*bn)) bn = bn -> parent;
        return bn -> parent;
    }
}
...
template<typename T>
BinNodePosi(T) BinTree<T>::attachAsLc(BinNodePosi(T) x, BinTree<T>* &S){// 注意这里是指针的引用
    if(x -> lc = S -> _root) S -> _root -> parent = x;//注意 T 有可能是空树
    _size += S -> _size;
    updateHeightAbove(x);
    release(S); S = NULL;//指针的引用的用处在这里
    return x;
}

template<typename T>
BinNodePosi(T) BinTree<T>::attachAsRc(BinNodePosi(T) x, BinTree<T>* &S){// 注意这里是指针的引用
    if(x -> rc = S -> _root) S -> _root -> parent = x;//注意 T 有可能是空树
    _size += S -> _size;
    updateHeightAbove(x);
    release(S); S = NULL;//指针的引用的用处在这里
    return x;
...
void BinTree<T>::travPre_I2(BinNodePosi(T) x, VST& visit){
    Stack<BinNodePosi(T)> s;
    s.push(x);
    while(!s.empty())   visitAlongLeftBranch(s, visit);
}
...
void BinTree<T>::travIn_I3(BinNodePosi(T) x, VST& visit){
    ...
    while(true)
    {
        if(!traceback && HasLChild(*x)) x = x -> lc;
        else
        {
            visit(x -> data);
            if(HasRChild(*x))
...
void BinTree<T>::print_structure(BinNodePosi(T) x, int n){
    ...
    if( !x ) std::cout << "|--X" << std::endl;
        ...
        if(n >= 0) std::cout << "|--" << x -> data << std::endl;
        else std::cout << "----------Structure of the binary tree:----------\n" << x -> data << std::endl;
    }
    if(x){
...
void BinTree<T>::print_structure_I(BinNodePosi(T) x){
    ...
    if(x)  std::cout << x -> data << std::endl;
    s.push(x -> lc); s.push(x -> rc);
    si.push(0); si.push(0);
...
bool BinTree<T>::check_cumsum_I(BinNodePosi(T) x, const T& e){
    ...
    while(!s.empty())
    {
        ...
        if(x)
        {
            if(x -> data < p) return false;
            s.push(x -> rc); s.push(x -> lc);
            st.push(p + x -> data); st.push(p + x -> data);
        }
    }
    return true;
}
...
void BinTree<T>::set_as_max_of_descendant(BinNodePosi(T) x){
    ...
    while(!s.empty())
    {
        if(s.top() != x -> parent) goToLeftmostLeaf(s);
        x = s.pop();
        if(HasBothChild(*x)) x -> data = (x -> lc -> data > x -> rc -> data)? x -> lc -> data : x -> rc -> data;
        else if (HasLChild(*x)) x -> data = x -> lc -> data;
        else if (HasRChild(*x)) x -> data = x -> rc -> data;
    }
}
...
void BinTree<T>::encode_by_level(){
    Stack<BinNodePosi(T)> s;
    _root -> data = 0;
    if(_root -> rc) s.push(_root -> rc);
    if(_root -> lc) s.push(_root -> lc);

    while(!s.empty())
    {
        BinNodePosi(T) x = s.pop();
        if(IsLChild(*x)) x -> data = x -> parent -> data * 2 + 1;
        else if(IsRChild(*x)) x -> data = x -> parent -> data * 2 + 2;
